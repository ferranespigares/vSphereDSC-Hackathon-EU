...
#region DSC Functions
    [void]Set()
    {
        ...
        if ($this.Ensure -eq [Ensure]::Present)
        {
            if(-not $alarmPresent)
            {
                Write-Verbose -Message "$(Get-Date) Creating Alarm $($this.Path)/$($this.Name)"
                $this.NewVmwAlarm()
            }
        }
        else
        {
            if ($alarmPresent)
            {
                Write-Verbose -Message "$(Get-Date) Removing Alarm $($this.Path)/$($this.Name)"
                $this.RemoveVmwAlarm()
            }
        }
    }
...
#region VmwAlarm Helper Functions
    [bool]TestVmwAlarm()
    {
        ...
        $alarmFound = Get-VmwNodeFromPath -Path $nodePath | where {$_.Found}

        Write-Verbose -Message "$(Get-Date) Found it ? $($dsFound -ne $null)" 
        return ($alarmFound -ne $null)
    ...
    [void]RemoveVmwAlarm()
    {
        ...
        $alarm = Get-VmwNodeFromPath -Path $nodePath

        # Take action on node
        if($alarm.Found){
            Remove-VmwAlarm -Alarm $alarm.Node | Out-Null
        }
        Write-Verbose -Message "$(Get-Date) $($s = Get-PSCallStack;"<Leaving {0}" -f $s[0].FunctionName)" 
    }
#endregion
...
#region VmwPermission Helper Functions
#endregion
}
